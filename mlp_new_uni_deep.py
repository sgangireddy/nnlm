...
class MLP(object):
    ...
    def __init__(self, rng, input1, input2, n_in, fea_dim, context_size, n_hidden1, n_hidden2, n_out, \
        ...
        self.hiddenLayer1 = HiddenLayer(rng = rng, input = self.projectionlayer.output, 
                                 n_in = fea_dim * context_size, n_out = n_hidden1,
                                 W_values = W_hid, b_values = b_hid,
	...
	self.hiddenLayer2 = HiddenLayer(rng = rng, input = self.hiddenLayer1.output,
        ...
        self.L1 = abs(self.hiddenLayer1.W).sum() + abs(self.hiddenLayer2.W).sum() \
                + abs(self.logRegressionLayer.W).sum() + abs(self.projectionlayer.W).sum() 

        # square of L2 norm ; one regularization option is to enforce 
        # square of L2 norm to be small
        self.L2_sqr = (self.hiddenLayer1.W**2).sum() + abs(self.hiddenLayer2.W**2).sum()\
                    + (self.logRegressionLayer.W**2).sum() + (self.projectionlayer.W**2).sum()
        ...
        self.likelihood = self.logRegressionLayer.likelihood_values
        ...
        self.params = self.logRegressionLayer.params + self.hiddenLayer2.params + self.hiddenLayer1.params + self.projectionlayer.params
        #self.delta_params = self.hiddenLayer.delta_params + self.hiddenLayer2.delta_params + self.logRegressionLayer.delta_params
