...
from learn_rates_modified import LearningRateNewBob, LearningRateList
from learn_rates_modified import LearningRate
...
opts, extraparams = getopt.getopt(sys.argv[1:], "f:c:h:p:m:l:r:e:v:", ["--feature", "--context", "--hidden", "--path", "--file_name1", "--file_name3", "--learn_rate", "--epoch", "--valid_loss"])
for o,p in opts:
  if o in ['-f','--feature']:
     feature_dimension = p
  elif o in ['-c', '--context']:
     ...
     file_name3 = p
  elif o in ['-r', '--learn_rate']:
     learn_rate = p
  elif o in ['-e', '--epoch']:
     epoch = p
  elif o in ['-v', '--valid_loss']:
     valid_loss = p


train_mlp(feature_dimension, context, hidden_size, weight_path, file_name1, file_name3, learn_rate, epoch, valid_loss)
